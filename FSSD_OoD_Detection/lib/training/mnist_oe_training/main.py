...
transform_test = trn.Compose([
    trn.RandomRotation((args.rotation,args.rotation)),
    trn.ToTensor(),
    # transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
])


train_data_in = dset.MNIST('./data', train=True, transform=transform_train)
test_data = dset.MNIST('./data', train=False, transform=transform_test)
num_classes = 10
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(net_state, './checkpoint/{}.pth'.format('mnist_oe'))
        best_acc = acc
